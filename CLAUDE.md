...
ALLOWED_EMAILS      - アクセス許可メールアドレス（カンマ区切り）※未設定時は全員アクセス可
```

## 完成している機能

- 物件一覧表示（サイト別バッジ・価格・住所・日付）
- 物件表示改善（価格二重表示修正・新築/中古/築年数バッジ・地名自動抽出）
- フィルター（状態・サイト・日付・テキスト検索・地域・価格帯）
- ページネーション（10件ごと）
- お気に入り登録
- 成約済みマーク・解除
...
- ログイン前に利用規約・プライバシーポリシーへの同意チェックボックス必須
- アクセス制限（ALLOWED_EMAILS環境変数によるホワイトリスト）
...
- HOME'Sコピー機能（タイトルをクリップボードにコピー→HOME'Sで検索）
- 利用規約・プライバシーポリシー・特定商取引法・お問い合わせフォーム（モーダル）
...
| ✅ 完了 | モバイルレスポンシブ対応 |
| ✅ 完了 | 利用規約・プライバシーポリシー・特定商取引法・お問い合わせフォーム |
| ✅ 完了 | ログイン前の利用規約・プライバシーポリシー同意チェック |
| ✅ 完了 | アクセス制限（ALLOWED_EMAILS環境変数によるホワイトリスト） |
| ✅ 完了 | 物件表示改善（価格二重表示修正・新築/中古バッジ・地名抽出） |
| ✅ 完了 | フィルター拡張（テキスト検索・地域・価格帯・ページネーション） |
| ✅ 完了 | HOME'Sコピー機能 |
| ✅ 完了 | SUUMO売買物件の価格抽出修正（HTMLリンク位置ベースに変更） |
| ✅ 完了 | お問い合わせメール送信修正（Resend API対応・Railway SMTP制限回避） |
| ⬜ 保留 | Stripe決済（有料化）※販売方針決定後に着手 |
| ⬜ 保留 | グーホーム・うちなーらいふ対応※各サイトに問い合わせ中 |
| ⬜ 未着手 | 会社情報ページ |

## 注意点

- `mail.js`は過去30日分のメールを取得する。取得後`process.exit()`で終了する設計。
...
- お問い合わせメールはResend API（resend.com）で送信。RailwayはSMTPポートをブロックするため。Resendアカウントのメールアドレス＝okinawa.realestate.notify@gmail.com。無料プランは月100通まで。
